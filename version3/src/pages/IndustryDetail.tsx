...
export default function IndustryDetail() {
  ...
  const p = ui.pages.industryDetail
  const industry = id ? content.getIndustry(id) : undefined

  if (!industry) return <Navigate to="/industries" replace />

  const related = industry.solutionIds.map((sid) => content.solutions.find((s) => s.id === sid)).filter(Boolean)
  const relatedSectors = content.sectors.filter((s) => industry.sectorIds.includes(s.id))
  const others = content.industries.filter((i) => i.id !== industry.id)

  return (
    <>
      <Seo title={industry.title} description={industry.description} />
      <PageHero eyebrow={ui.pages.industries.eyebrow} title={industry.title} lead={industry.description} back={{ to: '/industries', label: p.back }} />
              ...
              {related.map((s) => (
                <li key={s!.id} className="border-b border-paper-300">
                  <Link to={`/solutions/${s!.id}`} className="group flex items-center justify-between gap-6 py-5">
                    <span>
                      <span className="block font-display text-[20px] text-text transition-colors duration-300 group-hover:text-accent-700">{s!.title}</span>
      ...
      {relatedSectors.length > 0 && (
        <Section className="bg-paper-200 py-20 md:py-28">
          <div className="container-x">
            <Eyebrow>{p.sectorsInside}</Eyebrow>
