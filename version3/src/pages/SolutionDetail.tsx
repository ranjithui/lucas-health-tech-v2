...
export default function SolutionDetail() {
  ...
  const p = ui.pages.solutionDetail
  const solution = id ? content.getSolution(id) : undefined
  ...
  const related = content.industries.filter((i) => i.solutionIds.includes(solution.id))
  const others = content.solutions.filter((s) => s.id !== solution.id)
  ...
  return (
    <>
      ...
      />
      <PageHero eyebrow={solution.kicker} title={solution.fullTitle} lead={solution.description} back={{ to: '/solutions', label: p.back }} />
      ...
      <Section>
        <div className="container-x grid gap-16 lg:grid-cols-[1fr_1fr] lg:gap-24">
          <motion.div variants={stagger(0.08)} initial="hidden" whileInView="show" viewport={viewportOnce}>
            ...
            <motion.div variants={fadeUp} className="mt-8 flex flex-wrap gap-1.5" aria-label={ui.common.technologies}>
          ...
          <motion.div variants={stagger(0.08)} initial="hidden" whileInView="show" viewport={viewportOnce}>
            <motion.div variants={fadeUp}>
              <Eyebrow>{p.howItRuns}</Eyebrow>
            </motion.div>
            <motion.ol variants={fadeUp} className="mt-8 space-y-8">
              {solution.workflow.map((w, i) => (
                <li key={w.label} className="relative pl-12">
                  <span className="absolute left-0 top-0 font-mono text-[11px] tracking-[0.2em] text-accent-600">{String(i + 1).padStart(2, '0')}</span>
                  {i < solution.workflow.length - 1 && <span aria-hidden className="absolute left-[7px] top-6 h-[calc(100%+1rem)] w-px bg-paper-300" />}
            ...
            {solution.proof && (
              <motion.figure variants={fadeUp} className="mt-12 border-l border-accent-600 pl-6">
                <span className="label">{ui.common.evidence}</span>
