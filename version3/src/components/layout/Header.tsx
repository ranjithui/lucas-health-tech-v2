 ...
 *
 * `atTop` is about the background; `light` is about the text. The homepage
 * hero is a video under a navy wash in both themes, so on "/" the bar starts
 * with white type regardless of theme (`overVideo`).
 */
export function Header({ overHero }: { overHero: boolean }) {
  ...
  const light = (atTop && (theme === 'dark' || overVideo)) || open

  /** Minimal by design — four destinations and one call to action. */
  const navItems = [
    { label: ui.nav.solutions, to: '/solutions' },
    { label: ui.nav.industries, to: '/industries' },
    ...
    <>
      ...
      >
        <div className={cn('container-x flex items-center justify-between transition-all duration-500', scrolled ? 'h-[64px]' : 'h-[92px]')}>
          ...
          <nav className="hidden items-center gap-8 md:flex" aria-label={ui.nav.primary}>
            {navItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
          ...
          <div className="hidden items-center gap-3 md:flex">
            ...
            <Button to="/contact" size="sm" icon variant={light ? 'inverse' : 'primary'} className="ml-1">
            ...
            >
              ...
              >
                ...
                <div className="flex items-center justify-between font-mono text-[11px] uppercase tracking-[0.2em] text-white/40">
                  <a href={content.company.phoneHref} className="hover:text-white">
