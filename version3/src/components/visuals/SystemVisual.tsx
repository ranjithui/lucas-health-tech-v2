...
/**
 * The animated system flow: healthcare is the field, drawn as a field.
 *
 ...
 * Labels come from the active language.
 */
...
type NodeId = 'healthcare' | 'clinical' | 'operations' | 'technology' | 'innovation'
interface Node {
  id: NodeId
  role: Role
  deg: number
...
const NODES: Node[] = [
  { id: 'healthcare', role: 'field', deg: -90, side: 'top', Icon: Users },
  { id: 'clinical', role: 'domain', deg: -150, side: 'left', Icon: Stethoscope },
  { id: 'operations', role: 'domain', deg: -30, side: 'right', Icon: Settings },
  { id: 'technology', role: 'domain', deg: 150, side: 'left', Icon: Cloud, late: true },
  { id: 'innovation', role: 'domain', deg: 30, side: 'right', Icon: Lightbulb, late: true },
]
...
const PALETTE = {
  dark: {
    ...
    particle: '#8fb3e0',
    ring: 'rgba(143,179,224,0.5)',
    stream: '#3b6ea8',
    flow: '#b4cceb',
    pulse: '#ffffff',
    hubFill: '#0f1e30',
    hubCore: '#1a3352',
    hubEdge: 'rgba(143,179,224,0.55)',
    hubGlow: 'rgba(31,78,140,0.55)',
    nodeFill: '#152840',
  ...
  light: {
    ...
    particle: '#3b6ea8',
    ring: 'rgba(31,78,140,0.45)',
    stream: '#1f4e8c',
    flow: '#3b6ea8',
    pulse: '#1f4e8c',
    hubFill: '#ffffff',
    hubCore: '#ffffff',
    hubEdge: 'rgba(31,78,140,0.4)',
    hubGlow: 'rgba(59,110,168,0.32)',
    nodeFill: '#ffffff',
    nodeEdge: 'rgba(31,78,140,0.4)',
    icon: '#1f4e8c',
    label: '#0f1e30',
    sub: '#5c6b7a',
    baseOpacity: 0.75,
...
export function SystemVisual({ dark = true, className = 'max-w-[520px]' }: { dark?: boolean; className?: string }) {
  ...
  const labels = ui.systemVisual.nodes
  const T = dark ? PALETTE.dark : PALETTE.light
  const current = content.domains.find((d) => d.id === active)
  ...
  return (
    <div className={cn('relative mx-auto w-full', className)}>
      <div className="relative aspect-[680/570] w-full">
        <svg viewBox={`0 ${VB.y0} ${VB.w} ${VB.h}`} className="absolute inset-0 h-full w-full overflow-visible" role="img" aria-label={ui.systemVisual.aria}>
          <defs>
          ...
          <motion.g initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ ...spring, delay: 0.15 }} style={origin}>
            ...
            <text x={HUB.x} y={HUB.y + 38} textAnchor="middle" fill={T.sub} className="hidden font-mono sm:block" style={{ fontSize: 7.5, letterSpacing: '0.18em' }}>
              {ui.systemVisual.ourSystem.toUpperCase()}
            </text>
          </motion.g>
        ...
        {DOMAINS.map((n) => {
          const d = content.domains.find((x) => x.id === n.id)!
          const { x, y } = POS[n.id]
          return (
            <button
              key={n.id}
      ...
      <div className="mt-2 min-h-[52px] px-2 text-center sm:min-h-[44px]">
        <AnimatePresence mode="wait">
          ...
          >
            {current ? current.detail : ui.systemVisual.caption}
          </motion.p>
        </AnimatePresence>
      </div>
    </div>
