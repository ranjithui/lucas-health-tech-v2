      ...
      <div className="container-x">
        <motion.div variants={stagger(0.1)} initial="hidden" whileInView="show" viewport={viewportOnce} className="max-w-2xl">
          <motion.div variants={fadeUp}>
            <Eyebrow>{ui.home.whoWeServe.eyebrow}</Eyebrow>
          </motion.div>
          <motion.h2 variants={fadeUp} className="display-lg mt-7 text-balance">
            {ui.home.whoWeServe.title}
          </motion.h2>
        </motion.div>

        <motion.ul variants={stagger(0.08, 0.1)} initial="hidden" whileInView="show" viewport={viewportOnce} className="mt-14 grid gap-5 sm:grid-cols-2">
          {content.industries.map((ind) => (
            <InteractiveCard key={ind.id} as="li" variant="sweep" className="panel">
              <Link to={`/industries/${ind.id}`} className="flex h-full flex-col justify-between gap-10 px-7 py-10 sm:px-9 sm:py-12">
                <div>
                  <h3 className="font-display text-[24px] leading-tight text-text transition-colors duration-300 group-hover/card:text-accent-700">{ind.title}</h3>
