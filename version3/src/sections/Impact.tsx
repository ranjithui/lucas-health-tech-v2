  ...
  return (
    <Section id="impact" className="bg-mist-100">
      <div className="container-x">
        <Eyebrow>{ui.sections.impact.eyebrow}</Eyebrow>
        ...
        <div className="mt-14 mask-fade-x overflow-hidden" aria-label={ui.common.technologiesAndPlatforms}>
          <div className="flex w-max animate-marquee gap-2.5 hover:[animation-play-state:paused]">
            {[...technologies, ...technologies].map((t, i) => (
              <span key={`${t}-${i}`} className="rounded-full border border-paper-300 bg-paper-50 px-4 py-2 font-mono text-[11.5px] tracking-wide text-muted">
                {t}
              </span>
            ))}
          </div>
